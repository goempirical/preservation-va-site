...
class Give_Authorize_Payments {
	...
	public function hooks() {
		...
		add_filter( 'give_payment_details_transaction_id-authorize_echeck', array( $this, 'link_transaction_id' ), 10, 2 );
	...
	function setup_webhooks( $login_id, $transaction_key, $server ) {
		...
		try {
			...
			$webhooks_check = $request->getWebhooks();
			...
			// Ok, not create the webhooks for this URL.
	...
	public function process_webhooks( $event_json ) {
		...
		if ( isset( $event_json->eventType ) ) {
			...
			if (
				...
				$payment_id = $wpdb->get_var( $wpdb->prepare( "SELECT post_id FROM $wpdb->postmeta WHERE meta_key = '_give_payment_transaction_id' AND meta_value = %s LIMIT 1", $transaction_id ) );
	...
	public function process_payment( $donation_data ) {
		...
		$args = apply_filters( 'give_authorize_payment_args', array(
			...
			'transactionRequest' => array(
				...
				'billTo'              => array(
					...
					'country'   => $donation_data['card_info']['card_state'],
				),
		...
		try {
			...
			if ( $response->isSuccessful() && $response->isApproved() ) {
				...
				give_set_error( 'authorize_request_error', sprintf(__( 'The donation could not be charged. Please try again. Reason: %s', 'give-authorize' ), $error_message ) );
	...
	public function bank_billing_fields( $form_id ) {
		...
		<fieldset id="give_checkout_bank_details">
			...
			<div class="ga-column-wrap">
				...
				<p>
					<label><?php esc_html_e( 'Routing Number', 'give-authorize' ); ?></label>
					<input type="text" name="routing-number" placeholder="<?php echo esc_attr( 'Routing Number', 'give-authorize' ); ?>">
				</p>

				<!-- Account Number input field -->
				<p>
					<label><?php esc_html_e( 'Account Number', 'give-authorize' ); ?></label>
					<input type="text" name="account-number" placeholder="<?php echo esc_attr( 'Account Number', 'give-authorize' ); ?>">
				</p>
			...
			<div class="ga-column-wrap">
				...
				<p>
					<label><?php esc_html_e( 'Name on Account', 'give-authorize' ); ?></label>
					<input type="text" name="name-on-account" placeholder="<?php echo esc_attr( 'Name on Account', 'give-authorize' ); ?>">
				</p>


				<!-- Account Types select fields -->
				<p>
					<label><?php esc_html_e( 'Account Type', 'give-authorize' ); ?></label>
...
data-tooltip="' . __( 'This is the Authorize.net transction ID. Click the link to search for on the gateway.', 'give-authorize' ) . '"></span>';
