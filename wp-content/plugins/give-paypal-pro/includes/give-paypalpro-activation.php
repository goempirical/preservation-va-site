...
function give_paypal_pro_activation_banner() {
	...
	if ( is_admin() && current_user_can( 'activate_plugins' ) && ! $is_give_active ) {
		...
		add_action( 'admin_notices', 'give_paypal_pro_activation_notice' );

		//Don't let this plugin activate
		deactivate_plugins( GIVEPP_BASENAME );
	...
	if ( defined( 'GIVE_VERSION' ) && version_compare( GIVE_VERSION, GIVEPP_MIN_GIVE_VERSION, '<' ) ) {
		...
		add_action( 'admin_notices', 'give_paypal_pro_min_version_notice' );
	...
	if ( ! class_exists( 'Give_Addon_Activation_Banner' )
		...
		$args = array(
			...
			'documentation_url' => 'https://givewp.com/documentation/add-ons/paypal-pro-gateway/',
 ...
 * @since 1.1.3
 */
function give_paypal_pro_activation_notice() {
	echo '<div class="error"><p>' . __( '<strong>Activation Error:</strong> You must have the <a href="https://givewp.com/" target="_blank">Give</a> plugin installed and activated for the PayPal Pro add-on to activate.', 'give-paypal-pro' ) . '</p></div>';
}
 ...
 * @since 1.1.3
 */
function give_paypal_pro_min_version_notice() {
	echo '<div class="error"><p>' . sprintf( __( '<strong>Activation Error:</strong> You must have <a href="%s" target="_blank">Give</a> version %s+ for the PayPal Pro add-on to activate.', 'give-paypal-pro' ), 'https://givewp.com', GIVEPP_MIN_GIVE_VERSION ) . '</p></div>';
}
