...
if ( ! defined( 'GIVE_RECURRING_VERSION' ) ) {
	define( 'GIVE_RECURRING_VERSION', '1.7.1' );
}
if ( ! defined( 'GIVE_RECURRING_MIN_GIVE_VERSION' ) ) {
	define( 'GIVE_RECURRING_MIN_GIVE_VERSION', '2.1.7' );
}
...
final class Give_Recurring {
	...
	private function includes_global() {
		...
		// Find and include gateways.
		foreach ( give_get_payment_gateways() as $gateway_id => $gateway ) {
			if ( file_exists( sprintf( '%s/includes/gateways/give-recurring-%s.php', GIVE_RECURRING_PLUGIN_DIR, $gateway_id ) ) ) {
				require( sprintf( '%s/includes/gateways/give-recurring-%s.php', GIVE_RECURRING_PLUGIN_DIR, $gateway_id ) );
			}
		}
	...
	public function modify_donation_data( $payment_meta, $valid_data ) {
		...
		if (
			...
			$payment_meta['frequency'] = Give_Recurring::get_interval( $form_id, $price_id );
	...
	static function periods( $number = 1, $period = '' ) {
		...
		$periods = apply_filters( 'give_recurring_periods',
			array(
		...
		return ( ! empty( $period ) ) ? $periods[ $period ] : $periods;
	...
	public static function get_gateway_class( $gateways_id ) {
		return array_key_exists( $gateways_id, Give_Recurring::$gateways ) ?
			Give_Recurring::$gateways[ $gateways_id ] :
			'';
	}
