	...
	public function add_give_subscription_endpoint( $data, $query_mode, $api_object ) {
		...
		if ( $subscriptions ) {
			...
			foreach ( $subscriptions as $subscription ) {
				...
				if ( isset( $response_data['subscriptions'][ $count ]['info']['product_id'] ) ) {
					unset( $response_data['subscriptions'][ $count ]['info']['product_id'] );
				}

				if ( isset( $response_data['subscriptions'][ $count ]['info']['customer_id'] ) ) {
					unset( $response_data['subscriptions'][ $count ]['info']['customer_id'] );
				}
