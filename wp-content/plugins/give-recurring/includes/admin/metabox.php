							...
							<?php foreach ( $payments as $payment ) : ?>
								<li>
									...
									<span><?php echo date_i18n( get_option( 'date_format' ), strtotime( $payment->post_date ) ); ?>
										&nbsp;&ndash;&nbsp;</span>
									<span><?php echo give_donation_amount( $payment->ID, array(
										'currency' => true,
										'amount' => true,
									) ); ?></span>
								</li>
...
function give_recurring_show_parent_payment_table_icon( $value, $payment_id, $column_name ) {
	...
	$subscription = new Give_Subscription();
	$status       = $subscription->is_parent_payment( $payment_id );
	...
	if ( $status == true ) {
		...
		switch ( $column_name ) {
			...
			case 'status' :
				...
				$value = $value . '<span class="give-donation-status-recurring give-tooltip" data-tooltip="' . __( 'This is a recurring donation and the initial payment made by this donor. All payments made hereafter for the profile are marked as renewal payments.', 'give-recurring' ) . '"> ' . give_recurring_symbol_img() . ' </span>';
...
function give_recurring_options_save( $post_id ) {
	...
	if ( isset( $_POST['_give_recurring'] ) && ! in_array( $_POST['_give_recurring'], array( 'yes_donor', 'yes_admin' ), true ) ) {
		return false;
	}
