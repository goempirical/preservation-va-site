...
/**
 * Class Give_Subscription_Completed_Email
 */
class Give_Subscription_Completed_Email extends Give_Email_Notification {
	...
	public function init() {
		...
		add_action( 'give_subscription_completed', array( $this, 'setup_email_notification' ), 10, 2 );
	}
	...
	public function get_email_subject( $form_id = 0 ) {
		$message = Give_Email_Notification_Util::get_value(
			$this,
			'subscription_completed_subject',
			$form_id,
	...
	public function get_email_message( $form_id = 0 ) {
		$message = Give_Email_Notification_Util::get_value(
			$this,
			'subscription_completed_message',
			$form_id,
	...
	public function get_setting_fields( $form_id = 0 ) {
		...
		$settings = array_merge(
			...
			array(
				array(
					'name'    => __( 'Subscription Completed Subject', 'give-recurring' ),
					'id'      => 'subscription_completed_subject',
				...
				array(
					'name'    => __( 'Subscription Completed Message', 'give-recurring' ),
					'id'      => 'subscription_completed_message',
	...
	public function setup_email_notification( $subscription_id, Give_Subscription $subscription ) {
		...
		if ( $sent ) {
			Give_Recurring_Emails::log_recurring_email( 'completed', $subscription, $this->get_email_subject() );
		}
...
return Give_Subscription_Completed_Email::get_instance();
