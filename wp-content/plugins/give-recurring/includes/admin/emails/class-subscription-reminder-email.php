...
class Give_Subscription_Reminder_Email extends Give_Email_Notification {
	...
	public function load_notice_edit_screen() {
		...
		/**
		 * Get all the email notices, i.e.; renewal and expiration notices.
		 */
		$stored_notices = get_option( 'give_recurring_reminder_notices', array() );

		/**
		 * Get values for the fields from options when you're
		 * on the 'edit' screen of Renewal or Expiration.
		 */
		if ( 'edit' === $this->notice_action && false !== $this->notice_id ) {

			/**
			 * If notice ID is more than the number of existing
			 * notices, then proceed.
			 */
			if ( $this->notice_id < $this->notice_count ) {
				$fields = array(
					'default_email_subject' => $stored_notices[ $this->notice_id ]['subject'],
					'default_email_message' => $stored_notices[ $this->notice_id ]['message'],
				);
			}
		} elseif ( 'add' === $this->notice_action ) {

			$this->verify_action_nonce( 'add' );

			/**
			 * Default value of subject and message fields.
			 * This is visible when you are trying to add
			 * a new Reminder.
			 */
			 ...
			 * a new Expiration.
			 */
			if ( 'expiration' === $this->notice_type ) {
				$fields = array(
	...
	public function get_email_message( $form_id = 0 ) {
		...
		$this->config['content_type'] = $stored_notices[ $this->notice_id ]['content_type'];
	...
	public function setup_email_data() {
		Give()->emails->__set( 'heading', esc_html__( 'Subscription Reminder', 'give-recurring' ) );
	}
	...
	public function add_email_fields_for_renewal_reminder() {
		...
		$this->settings[] = array(
			...
			'default' => ( 'edit' === $this->notice_action ) ? $stored_notices[ $this->notice_id ]['status'] : 'enabled',
		);
		...
		$this->settings[] = array(
			...
			'default' => ( 'add' === $this->notice_action ) ? 'text/html' : $stored_notices[ $this->notice_id ]['content_type'],
		);
		...
		$this->settings[] = array(
			...
			'default'    => ( 'edit' === $this->notice_action ) ? $stored_notices[ $this->notice_id ]['gateway'] : '',
			'attributes' => array(
	...
	public function disable_settings_save_button() {
		$is_subscription_page = 'subscription-reminder' === give_get_current_setting_section() ? true : false;
		$is_notice_type       = ( 'renewal' === $this->notice_type || 'expiration' === $this->notice_type ) ? true : false;
		$is_notice_action     = ( 'add' === $this->notice_action || 'edit' === $this->notice_action ) ? true : false;

		if ( $is_subscription_page && $is_notice_type && $is_notice_action ) {
			return true;
		}
