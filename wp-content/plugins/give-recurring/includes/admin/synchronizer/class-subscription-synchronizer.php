 ...
 *
 * @see https://github.com/WordImpress/Give-Recurring-Donations/issues/118
 *
...
class Give_Subscription_Synchronizer {
	...
	function normalize_gateway_period( $gateway_period ) {
		...
		$weekly_array  = array( 'week' );
		$monthly_array = array( 'month', 'mont' );
		$yearly_array  = array( 'year' );
	...
	public function sync_subscription_transactions( $subscription, $should_log = true ) {
		...
		foreach ( $gateway_transactions as $key => $gateway_transaction ) {
			...
			if ( $this->can_sync_transaction( $gateway_transaction ) ) {
				...
				$this->print_notice( sprintf( __( 'A donation made on %s is missing and has been added.', 'give-recurring' ), $transaction_date ), 'item_missing' );
				...
				// This payment exists.
				$this->print_notice( sprintf( __( 'The donation made on %s is already recorded.', 'give-recurring' ), $transaction_date ), 'item_present' );
		...
		if ( $child_payments > 0 ) {
			$this->print_notice( sprintf( __( '%s renewal donations are in sync.', 'give-recurring' ), $child_payments ), 'success' );
		}
	...
	private function can_sync_transaction( $gateway_transaction ) {
		...
		return apply_filters( 'can_sync_transaction', $ret, $gateway_transaction );
