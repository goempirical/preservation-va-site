...
class Give_Recurring_PayPal_Pro_Payflow extends Give_Recurring_Gateway {
	...
	public function process_web_accept_ipn( $ipn_data, $txn_type ) {
		...
		$ipn_time      = strtotime( $ipn_data['payment_date'] );
		...
		foreach ( $subscriptions as $subscription ) {
			...
			if (
				...
				// To do this we convert the payment timestamp back to PST.
	...
	function record_signup() {
		...
		$args = array(
			...
			'expiration'        => $subscriber->get_new_expiration( $this->subscriptions['id'], $this->subscriptions['price_id'], $frequency ),
	...
	public function get_subscription_details( $subscription ) {
		...
		// Payflow doesn't return billing_period if the subscription is expired.
		// So we just default to what the subscription is to prevent errors + support.
		if ( 'expired' === $status ) {
